...
write_rds(list(bill_tree_mat = bill_tree_mat,
               ...
               bill_tips = bill_tips,
               blens = blens), "data/dataloader_dat.rds")

bill_ds <- bill_dataset(bill_tree_mat, start_mat, bill_tree_mat_tips, bill_tips, blens)
bill_dl <- dataloader(bill_ds, 112, shuffle = TRUE, drop_last = FALSE)
...
mod <- mani_evo_mod(n_rates = nrow(bill_init_rates), n_dim = 16,
                    ...
                    evenness_weight = 1,
                    n_segs = 100, init_rate = bill_init_rates,
                    device = "cuda")
mod <- mod$cuda()

write_rds(as.numeric(mod$target_rho$cpu()), "data/estimated_rho_16dim_rho_schedule.rds")
...
torch_save(mod, "data/mani_evo_mod_run_new_full_param_16dim_rho_scedule.to")

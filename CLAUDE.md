...
The current active development direction models beak shape evolution on the **Riemannian manifold** implied by the VAE latent space, using non-Euclidean distances rather than assuming the latent space is flat. This is the `.VAE_*` script series described below.
